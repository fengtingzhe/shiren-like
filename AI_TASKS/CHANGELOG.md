...
在 v0.4 镜头与画面表现调整版基础上，不新增玩法，改回更适合键盘操作的传统轻斜俯视镜头。

---

### 已完成

- [x] 将 `Builds/web-demo/game.js` 的核心镜头切换为 `traditional-tilt`；
- [x] 将 `tileToScreen` / `screenToTile` 从等距公式改为传统轻斜俯视坐标；
- [x] 将主要地图格从菱形改为轻斜俯视矩形格；
- [x] 保留轻微 `yScale` 压缩和 `perspectiveOffset`，维持纵深感；
- [x] 保留蓝色行动范围和红色威胁范围，并适配矩形格；
- [x] 保留 v0.4 的左上状态区、右上小地图、右侧敌人面板和底部快捷栏；
- [x] 保留 v0.3 的随机迷宫、饥饿、怪物追击、回复药、食物、传送卷轴、睡眠卷轴、火球杖和换位杖；
- [x] 将 `Data/config/web_demo_balance.json` 升级为 v0.4.1，并加入 `camera` 参数；
...
建议先完成 v0.4.1 试玩反馈，再决定 v0.5 的第一批规则怪物。
